...
// ServiceDataSource returns the service data source
...
func (d *serviceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if !state.ServiceIDs.IsNull() {
		serviceIDs := make([]string, 0)
		diags.Append(state.ServiceIDs.ElementsAs(ctx, &serviceIDs, true)...)
		...
		for _, service := range serviceDetails {
			serviceModel = append(serviceModel, helper.GetDataSourceServiceState(service))
		}
	}

	state.ServiceDetails = serviceModel

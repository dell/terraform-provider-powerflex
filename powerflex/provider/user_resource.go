...
func (r *userResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Error getting the system",
			"Could not get the system, unexpected err: "+err.Error(),
		)
		return
	}
	...
	if err2 != nil {
		resp.Diagnostics.AddError(
			...
			"Could not create user, unexpected error: "+err2.Error(),
		)
		return
	}
	...
	if err3 != nil {
		resp.Diagnostics.AddError(
			"Error fetching the user after creation",
			"Could not fetch user, unexpected error: "+err3.Error(),
		)
		return
	}
...
func (r *userResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			fmt.Sprintf("Could not get user by ID %s", state.ID.ValueString()),
			err.Error(),
		)
		return
	}
...
func (r *userResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err2 != nil {
		resp.Diagnostics.AddError(
			"Error Deleting User",
			"Couldn't Delete User "+err2.Error(),
		)
		return
	}
	if resp.Diagnostics.HasError() {

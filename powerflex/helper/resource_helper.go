		...
		if destinationField.IsValid() && destinationField.CanSet() {
			...
			case basetypes.Int64Value:
				...
				if destinationField.Kind() == reflect.Int64 {
					destinationField.Set(reflect.ValueOf(intVal.ValueInt64()).Convert(destinationField.Type()))
				}
...
func getFieldByJSONTag(destination interface{}, tag string) (reflect.Value, error) {
	...
	for i := 0; i < destElemType.NumField(); i++ {
		...
		// an expedient solution to make sure tfsdk and json tag can match, so that value assignment works
		if strings.EqualFold(strings.ReplaceAll(jsonTag, "_", ""), strings.ReplaceAll(tag, "_", "")) {
